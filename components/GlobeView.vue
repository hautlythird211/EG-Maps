...
function rebuildMarkers() {
  ...
  if (activeDataset.value === 'project-grants') {
    const data = isMobile.value
      ...
      : projectsData.value
    validProjects = data.filter(p => isValidCoordinate(p.latitude, p.longitude))
    ...
    const data = isMobile.value
      ...
      : speciesData.value
    speciesToRender = data.filter(s => isValidCoordinate(s.lat, s.lng))
  ...
  if (clusters.length === 0) {
    finishClusterRebuild(currentZoom)
    return
  }
  ...
  }, CLUSTER_CHUNK, () => finishClusterRebuild(currentZoom))
}

function finishClusterRebuild(currentZoom: number) {
