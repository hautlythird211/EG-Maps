...
export function setupEnterpriseLayer(map: MapLibreMap, onClick?: (e: EnterpriseHQ) => void) {
  ...
  // Add DOM markers for larger click targets
  ...
  // Connection lines between connected enterprises
...
export function cleanupEnterpriseLayer(map: MapLibreMap) {
  ...
  hqMarkers = []
  const layers = [ENTERPRISE_CONN_GLOW, ENTERPRISE_CONN_LAYER, ENTERPRISE_GLOW, ENTERPRISE_LAYER, ENTERPRISE_LABEL]
  layers.forEach(id => { try { map.removeLayer(id) } catch { /* empty */ } })
  const sources = [ENTERPRISE_CONN_SOURCE, ENTERPRISE_SOURCE]
  sources.forEach(id => { try { map.removeSource(id) } catch { /* empty */ } })
  mapInstance = null
}
